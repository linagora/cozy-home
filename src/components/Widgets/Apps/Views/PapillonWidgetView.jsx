...
export const PapillonWidgetView = ({ app, layoutControls, index }) => {
  ...
  const currentAccount =
    accounts && accounts.length > 0 ? accounts[accounts.length - 1] : null
...
export const PapTimetable = ({ currentAccount }) => {
  ...
  if (!courses || courses.length === 0) {
    return <UnimplementedWidgetView label={t('Widget.Papillon.NoCourses')} />
  }
  ...
  return (
    <List dense style={{ padding: 0 }}>
      {courses &&
        courses.length > 0 &&
        courses.map(course => (
          <PapTimetableItem key={course.id} client={client} course={course} />
        ))}
    </List>
  )
}
...
export const PapTimetableItem = ({ client, course, clock }) => {
  const openURL = useAppLinkWithStoreFallback(
    'papillon',
    client,
    `#/timetable/course/${course.id}`
  ).url
...
export const PapHomeworks = ({ currentAccount }) => {
  ...
  const homeworks = useMemo(() => {
    return homeworksData
      ? homeworksData.sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate))
      : []
  }, [homeworksData])
  ...
  if (!homeworks || homeworks.length === 0) {
    return <UnimplementedWidgetView label={t('Widget.Papillon.NoHomeworks')} />
  }
  ...
  return (
    <>
      {groupedHomeworks.map(dayGroup => (
        ...
        >
          {dayGroup.homeworks.map(homework => (
            <PapHomeworksItem
              key={homework.id}
              client={client}
              homework={homework}
...
export const PapHomeworksItem = ({ client, homework }) => {
  ...
  const trunactedSummary =
    homework.summary && homework.summary.length > 50
...
export const PapGrades = ({ currentAccount }) => {
  ...
  const grades = useMemo(() => {
    ...
    return timeseries
      .map(series =>
        series.series.map(grade => ({ ...grade, timeseries: series }))
      )
      .flat()
  ...
  if (!grades || grades.length === 0) {
    return <UnimplementedWidgetView label={t('Widget.Papillon.NoGrades')} />
  }

  return (
    <List dense style={{ padding: 0 }}>
      {grades.map(grade => (
        <GradeItem key={grade.id} client={client} grade={grade} />
      ))}
    </List>
  )
}
...
export const GradeItem = ({ client, grade }) => {
  const openURL = useAppLinkWithStoreFallback(
    'papillon',
    client,
    `#/grades/grade/${grade.timeseries._id}/${grade.id}`
  ).url
