            ...
            tabs={[
              {
                label: 'Fichiers',
                icon: 'clock',
                render: <DriveWidgetFileTab setOpenedFile={setOpenedFile} />
              },
              {
                label: 'Partages',
                icon: 'share',
                render: (
                  <DriveWidgetSharingsTab
              ...
              },
              {
                ...
                icon: 'star',
                render: (
                  <DriveWidgetFavoritesTab setOpenedFile={setOpenedFile} />
                )
              }
      ...
      {openedFile.files.length > 0 && (
        <Viewer
          ...
          onChangeRequest={nextPhoto => {
            const photo = openedFile.files.findIndex(
              f => f._id === nextPhoto._id
            )
...
export const DriveWidgetFileTab = ({ setOpenedFile }) => {
  ...
  if (!files || files.length === 0) {
    return <UnimplementedWidgetView label={t('Widget.Drive.NoRecentFiles')} />
  }
...
export const DriveWidgetFoldersTab = () => {
  ...
  if (!folders || folders.length === 0) {
    return <UnimplementedWidgetView label={t('Widget.Drive.NoRecentFolders')} />
  }
...
export const DriveWidgetSharingsTab = ({
  sharedDocumentIds = [],
  setOpenedFile
}) => {
  const client = useClient()
  ...
  if (!files || files.length === 0) {
    return <UnimplementedWidgetView label={t('Widget.Drive.NoSharedFiles')} />
  }
...
export const DriveWidgetFavoritesTab = ({ setOpenedFile }) => {
  ...
  if (!files || files.length === 0) {
    return <UnimplementedWidgetView label={t('Widget.Drive.NoFavoriteFiles')} />
  }
...
export const WidgetDriveFolderItem = ({ folder, client }) => {
  ...
  return (
    <ListItemByDoc
      ...
      primary={<Typography variant={'subtitle1'}>{folder.name}</Typography>}
      secondary={
        <Typography variant="caption" color="textSecondary">
          {new Date(folder.cozyMetadata.updatedAt).toLocaleDateString(
            undefined,
            {
              year: 'numeric',
              month: 'short',
